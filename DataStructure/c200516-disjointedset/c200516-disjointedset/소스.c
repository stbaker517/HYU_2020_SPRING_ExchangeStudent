...
void Insert(BTTree* Tree, DATATYPE Data, DATATYPE Parent)
{	//adress setting
	AddressNode* dummy = (AddressNode*)malloc(sizeof(AddressNode));
	dummy->Data = Data;
	dummy->Parent = Parent;
	dummy->next = NULL;
	if (Tree->Head->next == NULL && Tree->Tail == NULL) {
		Tree->Head->next = dummy;
	}
	else
	{
		Tree->Tail->next = dummy;
	}
	Tree->Tail = dummy;
	Tree->NumofData++;
}
...
DATATYPE MakeAddressMapWithSubTree(BTTree* Tree, BTNode* Node)
{
	/*	int data;
		...
		else if (Node->Left != NULL && Node->Right == NULL)//left ok
		{
			data = MakeAddressMapWithSubTree(Tree, Node->Left);
			Insert(Tree, data, Node->Data);
			return Node->Data;
		}
		else if (Node->Left == NULL && Node->Right != NULL)//right ok
		{
			data = MakeAddressMapWithSubTree(Tree, Node->Right);
			Insert(Tree, data, Node->Data);
			return Node->Data;
		}
		else//both ok
	...
	if (Node == NULL)
		return NULL;
	LeftN = RetSubTreeLeft(Node);
	RightN = RetSubTreeRight(Node);

	if (LeftN != NULL && RightN != NULL)
	{
		left = MakeAddressMapWithSubTree(Tree, LeftN);
		Insert(Tree, left, Node->Data);
		right = MakeAddressMapWithSubTree(Tree, RightN);
		Insert(Tree, right, Node->Data);
		return Node->Data;
	}
...
int Union(BTTree* Tree1, BTTree* Tree2)
{
	...
	int HofT1 = Search(Tree1, &Tree1->Root->Data), HofT2 = Search(Tree2, &Tree2->Root->Data);
	DATATYPE tdata;
	...
	{//HofT2 into HofT1
		...
		PosHead(Tree2, &tdata);
		RemoveBST(&Tree2->Root, tdata);
		InsertBST(&Tree1->Root, tdata);
		while (PosNext(Tree2, &tdata)) {
			//tree node moving
			RemoveBST(&Tree2->Root, tdata);
			InsertBST(&Tree1->Root, tdata);
		}
		//deallocate tree and linkedlist
	...
	else//t2 high
	{
		...
		PosHead(Tree1, &tdata);
		RemoveBST(&Tree1->Root, tdata);
		InsertBST(&Tree2->Root, tdata);
		while (PosNext(Tree1, &tdata)) {
			//tree node moving
			RemoveBST(&Tree1->Root, tdata);
			InsertBST(&Tree2->Root, tdata);
		}
